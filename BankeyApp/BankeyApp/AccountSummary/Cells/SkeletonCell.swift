...
private extension SkeletonCell {
    ...
    func setupLayers() {
        typeLayer.startPoint = CGPoint(x: 0.0, y: 0.5)
        typeLayer.endPoint = CGPoint(x: 1.0, y: 0.5)
        typeLabel.layer.addSublayer(typeLayer)
        
        nameLayer.startPoint = CGPoint(x: 0.0, y: 0.5)
        nameLayer.endPoint = CGPoint(x: 1.0, y: 0.5)
        nameLabel.layer.addSublayer(nameLayer)
        
        balanceLayer.startPoint = CGPoint(x: 0.0, y: 0.5)
        balanceLayer.endPoint = CGPoint(x: 1.0, y: 0.5)
        balanceLabel.layer.addSublayer(balanceLayer)
        
        balanceAmountLayer.startPoint = CGPoint(x: 0.0, y: 0.5)
        balanceAmountLayer.endPoint = CGPoint(x: 1.0, y: 0.5)
    ...
    func setupAnimation() {
        ...
        nameLayer.add(typeGroup, forKey: "backgroundColor")
        
        let balanceGroup = makeAnimationGroup(previousGroup: nameGroup)
        balanceLayer.add(typeGroup, forKey: "backgroundColor")
        
        let balanceAmountGroup = makeAnimationGroup(previousGroup: balanceGroup)
        balanceAmountLayer.add(typeGroup, forKey: "backgroundColor")
    }
